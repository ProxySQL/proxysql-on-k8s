...
The cluster reconciler is event-driven via watches on the CR and its owned
objects (StatefulSet, Services, Secrets, PodDisruptionBudget). Errors
requeue with controller-runtime's default backoff. The one timed requeue:
a TLS-enabled **tier-3** cluster returns `RequeueAfter ≈ serving-cert
NotAfter − renewBefore − now` (floored at 1 minute) so short custom
lifetimes cannot expire during an idle informer resync. Tiers 1 and 2 do
not set this — tier 2 is Secret-watch driven.
