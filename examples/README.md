...
| --- | --- | --- |
| [`mysql/oracle-mysql-operator/`](mysql/oracle-mysql-operator/) | Oracle's [MySQL Operator for Kubernetes](https://github.com/mysql/mysql-operator) (`mysql.oracle.com/v2.InnoDBCluster`) | `<pod>.<name>-instances` per-pod DNS (ProxySQL replaces Router) |
| [`mysql/percona-ps/`](mysql/percona-ps/) | [Percona Operator for MySQL Server](https://docs.percona.com/percona-operator-for-mysql/ps/) (`ps.percona.com/v1.PerconaServerMySQL`) | `<pod>.<name>-mysql` per-pod DNS, writer/reader split by `read_only` |
